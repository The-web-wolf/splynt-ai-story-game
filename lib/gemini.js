...
export async function generateStoryStep(currentGameState, gameSettings) {
  console.log(currentGameState,gameSettings)
  const gameProgress = JSON.stringify(currentGameState.progress)
  console.log("Progress from Gemini", gameProgress)
  const prompt = `
    ...
    - You may format your the story using HTML, but only with the following allowed tags: ${JSON.stringify(ALLOWED_HTML_TAGS)}
  `
  try {
    const result = await model.generateContent(prompt, { generationConfig })
    const text = result.response.text()
    console.log("Response", JSON.parse(text.trim()))
...
export async function generateExitConfirmation(currentGameState, gameSettings) {
  ...
  try {
    ...
    return { title: "Exit Interview?", body: "Harvey looks up as you stand to leave. Are you sure you want to go?" }
  }
}